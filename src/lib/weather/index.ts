...
export async function fetchAllSources(
  lat: number,
  lon: number,
  options?: { forceRefresh?: boolean; onPartial?: (partial: CourseWeather) => void }
): Promise<CourseWeather> {
  const key = cacheKey(lat, lon);
  const cached = cache.get(key);
  if (CACHE_ENABLED && !options?.forceRefresh && cached && cached.expiresAt > Date.now()) {
    return cached.data;
  }
...
export { aggregateForecasts, findCurrentPoint, HOUR_ROLLOVER_MINUTE, indexByHour } from './aggregate';
export * from './types';
