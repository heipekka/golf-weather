...
type Condition = 'clear' | 'partlyCloudy' | 'cloudy' | 'fog' | 'rain' | 'snow' | 'thunder' | 'unknown';
...
export function resolveCondition({
  ...
  if (precipitation !== null && precipitation !== undefined && precipitation > 0.2) {
    return 'rain';
  }
...
export function WeatherIcon({
  ...
  const icon = ICONS[resolveCondition({ weatherCode, precipitation, cloudCover })];
  const ios = isNight && icon.iosNight ? icon.iosNight : icon.ios;
  const material = isNight && icon.materialNight ? icon.materialNight : icon.material;
